...
const LatihanCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            tajuk: _entity?.tajuk,kategori: _entity?.kategori,status: _entity?.status,noRujukan: _entity?.noRujukan,
...
<div className="col-12 md:col-6 field mt-5">
            <span className="align-items-center">
                <label htmlFor="noRujukan">no rujukan:</label>
                <InputNumber id="noRujukan" className="w-full mb-3 p-inputtext-sm" value={_entity?.noRujukan} onChange={(e) => setValByKey("noRujukan", e.value)}  />
            </span>
            <small className="p-error">
